...
def importRawFloat(metadata, classes, options):
    if (metadata is None):
        return None
...
def importRawNumber(metadata, classes, options):
    return importRawFloat(metadata, classes, options)


def importRawInt(metadata, classes, options):
    if (metadata is None):
        return None
    if not metadata in classes.keys():
        classes[metadata] = int(metadata)
        classes['_length'] = classes.get('_length', 1) + 1
    return int(metadata), classes
