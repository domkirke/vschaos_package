...
from torch.distributions import TransformedDistribution, MultivariateNormal, transforms as transform
...
class NormalizingFlow(nn.Module):
    ...
    def __repr__(self):
        return "NormalizingFlow(block: %s, length: %s)"%(self.bijectors[0], len(self.bijectors))

    def __init__(self, dim, blocks, flow_length, density=None, amortized='none', amortize_dim=None):
        ...
        if amortized in ('self', 'input', 'auxiliary'):
            ...
            self.amortized_params = self.parameters_network(amortize_dim, self.n_parameters())
        self.log_det = []
        self.dim = dim
    ...
    def __getitem__(self, item):
        flow = NormalizingFlow(self.dim, self.blocks, density=self.base_density, flow_length=0)
        flow.blocks = self.blocks.__getitem__(item)
        flow.bijectors = self.bijectors.__getitem__(item)
        flow.transforms = transform.ComposeTransform(flow.bijectors.flows)
        flow.final_density = distrib.TransformedDistribution(self.base_density, flow.transforms)
        flow.amortized = self.amortized
        if self.amortized in ('self', 'input', 'auxiliary'):
            flow.amortized_seed = self.amortized_seed
            flow.amortized_params = self.amortized_params
        flow.n_params = self.n_params.__getitem__(item)
        return flow
    ...
    def amortization(self, z, aux=None):
        ...
        elif self.amortized == 'auxiliary':
            params = self.amortized_params(aux)
        self.set_parameters(params, z.shape[0])

    def forward(self, z, aux=None):
        """ Forward a set of samples and apply series of flows """
        self.log_det = []
        self.amortization(z, aux)
